...
from agentvast.paths import state_directory
from agentvast.tracker import LineageTracker
from agentvast.prov_dag import ProvDAG
...
def _get_default_base_dir() -> Path:
    ...
    return state_directory(Path(__file__).resolve().parent.parent)


class AgentVASTServer:
    """AgentVAST 服务器
    ...
    def record_step_details(self,
        ...
        try:
            from agentvast.step_details import record_step_details
    ...
    def get_step_details(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            from agentvast.step_details import StepDetailsRecorder
...
_server_instances: Dict[str, AgentVASTServer] = {}


def get_server(base_dir: str = None) -> AgentVASTServer:
    """获取服务器实例（支持多实例）
    ...
    if abs_dir not in _server_instances:
        _server_instances[abs_dir] = AgentVASTServer(base_dir=abs_dir)
