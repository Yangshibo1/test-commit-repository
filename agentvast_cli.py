...
用法：
    python agentvast_cli.py start              # 启动交互式追踪
    python agentvast_cli.py status            # 查看当前会话
    python agentvast_cli.py export <session>  # 导出会话
    python agentvast_cli.py list              # 列出所有会话
"""

import sys
import json
from pathlib import Path
from agentvast.mcp_server import AgentVASTServer, get_server
...
def main():
    ...
    if command == "start":
        ...
        if len(sys.argv) < 3:
            print("用法: agentvast_cli.py export <session_id>")
            return
