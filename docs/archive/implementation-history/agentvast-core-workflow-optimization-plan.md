...
AgentVAST 当前定位为个人使用、本地运行的数据分析 Agent 工作流记录系统。

第一目标宿主是 Claude Code。AgentVAST 通过 Claude Code Plugin 提供规则、MCP 工具和生命周期 Hook；后续可以为 OpenCode 等 Coding Agent 增加适配器，但核心记录模型不依赖某个 Agent。
...
- 一次用户数据分析任务对应一个 Run。
- 由 AgentVAST CLI 创建 Run 并启动交互式 Claude Code，确保记录先于 Agent 执行开始。
...
- 不重构现有前端交互和可视化。
- 不要求所有数据文件复制进 AgentVAST；默认只记录路径、版本和元数据。
...
当前 `agentvast/mcp_server.py` 主要是 Python 直接调用接口。要让 Claude Code 稳定使用，必须实现真正的 MCP Server，并通过插件自动加载工具。
...
1. `agentvast_start_run`
2. `agentvast_set_plan`
3. `agentvast_start_step`
4. `agentvast_complete_step`
5. `agentvast_revise_plan`
6. `agentvast_get_state`
7. `agentvast_finish_run`

人工介入增加两个工具：

8. `agentvast_classify_user_input`
9. `agentvast_apply_human_contribution`
...
第一版不以“用户先打开 Claude Code，再希望 Claude 自己想起使用 AgentVAST”为主流程。由 AgentVAST CLI 负责创建 Run 并启动 Claude Code：
...
4. 为子进程设置 `AGENTVAST_RUN_ID`、数据库路径和项目根目录。
5. 在项目目录启动交互式 Claude Code，并加载 AgentVAST Plugin。
...
claude
  ...
  --plugin-dir <agentvast-plugin-path>
  "<initial-prompt>"
```

启动器是 Claude Code 的父进程，但不接管 Claude 的分析循环。Claude 仍在用户熟悉的交互式终端中运行；AgentVAST 只负责在启动前建立 Run、向子进程传递上下文，并在退出后保存最终状态。
...
```text
agentvast resume <run-id>
```

CLI 根据 Run 中保存的 Claude Code session ID 恢复交互式会话，并设置相同 `AGENTVAST_RUN_ID`。`SessionStart` Hook 注入：
...
6. `PreToolUse` 在仍有未分类输入时阻止新的实质性操作，但允许 AgentVAST 分类工具和只读查询。
...
如果 Agent 一次运行一个“大脚本”，脚本内部完成加载、清洗、分析和报告，且执行期间 Agent 没有观察中间结果或作出新的分析决策，AgentVAST 必须如实记录为一个过粗 Step，并给出颗粒度警告。不能在执行后根据脚本结构虚构多个并未真实发生的语义 Step。
...
- 实现 `agentvast run` 和 `agentvast resume` 启动器。
...
- 用户通过一个 AgentVAST 命令进入已绑定 Run 的 Claude Code 会话。
- Claude Code 可以直接调用 AgentVAST 工具。
...
- AgentVAST 在启动 Claude Code 前已经创建 Run，并能用同一 session ID 恢复。
