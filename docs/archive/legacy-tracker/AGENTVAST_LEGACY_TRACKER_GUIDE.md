...
默认情况下，所有会话统一存储在项目根目录 `.agentvast/`。VAST Challenge 分析产生的 AgentVAST session 也写入这里；VAST 原始数据和中间业务数据仍保留在对应 VAST 数据目录。可通过 `AGENTVAST_BASE_DIR` 或 `get_server(base_dir)` 显式覆盖。
...
A: 默认统一存储在项目根目录下的 `.agentvast/` 文件夹。可以通过环境变量 `AGENTVAST_BASE_DIR` 或传入 `get_server(base_dir)` 自定义。
