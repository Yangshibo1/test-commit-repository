...
<main>
  <header>
    <div>
      ...
      <p class="subtitle">Complete chain from Emma Harbor creating SwiftWren.txt to John Windward's anomalous SaidIT post. Nodes show key events; arrows show chronological order (queue-labeled arrows also show delegation).</p>
    </div>
    <div class="stats">
  ...
  <section class="details">
    <div class="panel">
      ...
      <div id="deptActivity" class="mini-bars"></div>
    </div>
    <div class="panel">
      <h3>About</h3>
      <p style="color:var(--muted);font-size:11px;line-height:1.4"><b>Visual Encoding:</b> Nodes = events with person, department, and type; edges = chronological order (queue-labeled edges also show delegation); colors = departments; timeline = MST (UTC-7).</p>
      <p style="color:var(--muted);font-size:11px;line-height:1.4"><b>Key Events:</b> Events directly related to the anomalous post and their first-hop queue chains.</p>
      <p style="color:var(--muted);font-size:11px;line-height:1.4"><b>Interactions:</b> Toggle display modes (First 7+Last 7 / Show all / Key events); filter by department, event type, or person; search across all fields.</p>
    </div>
  </section>
</main>
<script>
...
function render() {{
  ...
  // Update department activity stats
  renderDeptBars();
}}
...
function renderDeptBars() {{
  ...
  document.getElementById('deptActivity').innerHTML = Object.entries(counts)
    .sort((a, b) => b[1] - a[1])
    .map(([dept, count]) => `<div class="bar"><span>${{dept}}</span><div class="bar-track"><div class="bar-fill" style="width:${{count/max*100}}%;--dept-color:${{COLORS[dept]?.border||'#ccc'}}"></div></div><b>${{count}}</b></div>`)
    .join('');
}}
